...
<style>
	...
	.rule-line {
		flex: 1;
		height: 1px;
		background: linear-gradient(to right, transparent, #4a4540);
	}

	.rule-line:last-child {
		background: linear-gradient(to left, transparent, #4a4540);
	}
	...
	/* ── CREDITS ── */
	.credits {
		padding: clamp(3rem, 8vh, 5rem) clamp(1.5rem, 5vw, 5rem) clamp(4rem, 10vh, 7rem);
		background: #080a0d;
